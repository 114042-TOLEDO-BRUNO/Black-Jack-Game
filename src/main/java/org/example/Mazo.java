...
public class Mazo {
    ...
    public void mezclarMazo(){
        List<Carta>mazoTemporal=new ArrayList<Carta>();
        //usamos random
        Random random=new Random();
        int randomIndiceDeCarta=0;
        int tamañoOriginal=this.cartas.size();
        for(int i=0;i<tamañoOriginal;i++){
            //generar el indice random rand.nextInt((max-min)+1)+min
            randomIndiceDeCarta=random.nextInt((this.cartas.size()-1-0)+1)+0;
            mazoTemporal.add(this.cartas.get(randomIndiceDeCarta));
            //borrar la carta del mazo original
            this.cartas.remove(randomIndiceDeCarta);
        }
